 ...
 */
 ...
 class Dynamic_site_menu
 {
	 ...
	 public function build_page($cur_id1,$cur_id2,$hmenu)
	 {
	 	$cur_id='';
						...
						if ($menu3->parent_id == 0)
						{
							...
							if ($menu3->is_parent == TRUE)
							{
								if ($menu3->page_id == $cur_id)
								{
									 ...
									 $html_out .= "\t\t".'<li class="dropdown menu-active"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">'.$menu3->page_name.'</a>';
								}
								else
								{
							...
							else
							{
								...
								{
									$html_out .= "\t\t\t".'<li>'.anchor($page_link, $menu3->page_name,array('class'=>'page-scroll'));
								}
							}
							$html_out .= '</li>'."\n";
						}
	...
	function get_childs($menu_array,$parent_id)
    {
		$has_subcats = FALSE;
        $html_out  = '';
        $html_out .= "\t\t\t\t".'<ul class="dropdown-menu submenu">'."\n";
        
       foreach ($menu_array as $menu3)
        {
			if ($menu3->parent_id == $parent_id)    // are we allowed to see this menu?
			{
				if($menu3->page_link == '')
				 {
					 $id=$menu3->page_id;
				 ...
				 else
				 {
					...
					$html_out .= "\t\t\t\t\t".'<li>'.anchor($page_link,$menu3->page_name);
				$html_out .= '</li>' . "\n";
			}
        }
        $html_out .= "\t\t\t\t\t".'</ul>' . "\n";
